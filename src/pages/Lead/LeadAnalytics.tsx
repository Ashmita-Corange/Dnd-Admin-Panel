...
import {
    TrendingUp, Users, Phone, CheckCircle, Calendar, Filter,
    BarChart3, PieChart, Activity, Search, X, Clock,
    DollarSign, AlertCircle, Sparkles
} from "lucide-react";
...
const LeadAnalytics: React.FC = () => {
    ...
    useEffect(() => { dispatch(fetchStaff({ limit: 100 })); }, [dispatch]);
    ...
    const MetricCard = ({ icon: Icon, value, label, gradient, iconBg, trend }: any) => (
        <div className="group relative overflow-hidden bg-gradient-to-br from-white to-gray-50 dark:from-gray-800/50 dark:to-gray-900/50 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 p-6 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
            ...
            <div className="relative">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {trend && <TrendingUp className="w-5 h-5 text-emerald-500 animate-pulse" />}
                </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900 p-6">
            ...
            <div className="mb-8 relative">
                ...
                <div className="relative">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 ml-11">
                        {isAdmin ? "Comprehensive analytics dashboard for all leads" : "Your personal performance metrics"}
                    </p>
                </div>
            </div>
                    ...
                    {isAdmin && (
                        <div>
                            ...
                            <div className="relative" ref={staffDropdownRef}>
                                <input type="text"
                                    value={selectedStaffMember ? `${selectedStaffMember.name} - ${selectedStaffMember.email}` : staffSearchTerm}
                                    onChange={(e) => { setStaffSearchTerm(e.target.value); setIsStaffDropdownOpen(true); }}
                                ...
                                <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                                    {selectedStaffMember ? (
                                        <button onClick={() => { setSelectedStaff(""); setStaffSearchTerm(""); }}
                                    ...
                                    <div className="absolute z-50 w-full mt-2 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                                        {filteredStaff.length > 0 ? (
                                            <>
                                                ...
                                                {filteredStaff.map(s => (
                                                    <div key={s._id} onClick={() => handleStaffSelect(s._id)}
                                                        ...
                                                        <span className="font-semibold text-gray-800 dark:text-white block">{s.name}</span>
            ...
            {loading && (
                <div className="flex flex-col items-center justify-center h-64 space-y-4">
                    ...
                    <p className="text-lg font-semibold text-gray-600 dark:text-gray-300 animate-pulse">Loading analytics...</p>
                </div>
            )}
            ...
            {!loading && analytics?.overview && (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        <MetricCard icon={Users} value={analytics.overview.totalLeads ?? 0} label="Total Leads"
                            gradient="from-blue-500 to-cyan-500" iconBg="bg-gradient-to-br from-blue-500 to-cyan-600 text-white" trend />
                        <MetricCard icon={Activity} value={analytics.overview.newLeads ?? 0} label="New Leads"
                            gradient="from-purple-500 to-pink-500" iconBg="bg-gradient-to-br from-purple-500 to-pink-600 text-white" />
                        <MetricCard icon={CheckCircle} value={analytics.overview.convertedLeads ?? 0} label="Converted Leads"
                            gradient="from-green-500 to-emerald-500" iconBg="bg-gradient-to-br from-green-500 to-emerald-600 text-white" />
                        <MetricCard icon={BarChart3} value={analytics.overview.conversionRate ?? "0.00%"} label="Conversion Rate"
                            gradient="from-orange-500 to-red-500" iconBg="bg-gradient-to-br from-orange-500 to-red-600 text-white" />
                    </div>

                    {/* Follow-up & Revenue */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        <MetricCard icon={AlertCircle} value={analytics.followUpMetrics?.untouchedLeads ?? 0} label="Untouched Leads"
                            gradient="from-red-500 to-rose-500" iconBg="bg-gradient-to-br from-red-500 to-rose-600 text-white" />
                        <MetricCard icon={Clock} value={analytics.followUpMetrics?.followupForToday ?? 0} label="Follow-ups Today"
                            gradient="from-yellow-500 to-amber-500" iconBg="bg-gradient-to-br from-yellow-500 to-amber-600 text-white" />
                        <MetricCard icon={Clock} value={analytics.overview.avgResponseTime ?? "N/A"} label="Avg Response Time"
                            gradient="from-indigo-500 to-blue-500" iconBg="bg-gradient-to-br from-indigo-500 to-blue-600 text-white" />
                        <MetricCard icon={DollarSign} value={`₹${(analytics.revenueMetrics?.totalExpectedRevenue ?? 0).toLocaleString()}`} label="Expected Revenue"
                            gradient="from-emerald-500 to-teal-500" iconBg="bg-gradient-to-br from-emerald-500 to-teal-600 text-white" trend />
                    </div>
                                    ...
                                    {Object.entries(analytics.statusDistribution).map(([status, count]) => (
                                        <div key={status} className="group">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 capitalize">{status}</span>
                                                <span className="text-lg font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">{count}</span>
                                            </div>
                                            <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                                <div className="h-full bg-gradient-to-r from-purple-500 to-pink-500 rounded-full transition-all duration-500 group-hover:scale-105"
                                                    style={{ width: `${(count / (analytics.overview?.totalLeads || 1)) * 100}%` }} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="space-y-4">
                                    {Object.entries(analytics.sourceBreakdown).map(([source, count]) => (
                                        <div key={source} className="group">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 capitalize">{source.replace(/_/g, " ")}</span>
                                                <span className="text-lg font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">{count}</span>
                                            </div>
                                            <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                                <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full transition-all duration-500 group-hover:scale-105"
                                                    style={{ width: `${(count / (analytics.overview?.totalLeads || 1)) * 100}%` }} />
                                            </div>
                                        </div>
                                    ))}
                    ...
                    {analytics.callMetrics && (
                        <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 rounded-3xl border border-gray-200/50 dark:border-gray-700/50 shadow-xl p-6 mb-8">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                                {Object.entries(analytics.callMetrics).map(([status, count]) => (
                                    <div key={status}
                                        className="p-5 bg-gradient-to-br from-gray-50 to-white dark:from-gray-700/50 dark:to-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700 hover:shadow-lg hover:scale-105 transition-all duration-300">
                                        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-2">
                                            {status.replace(/([A-Z])/g, " $1").trim()}
                                        </p>
                                        <p className="text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">{count}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
