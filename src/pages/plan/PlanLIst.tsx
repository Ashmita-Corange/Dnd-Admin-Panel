            ...
            {category.subCategoryCount > 0 && (
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md p-3 mb-4">
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  <strong>Warning:</strong> This tenant has{" "}
                </p>
              </div>
            )}
...
const TenantList: React.FC = () => {
    ...
    dispatch(
      fetchTenants({
        ...
        search: searchInput
          ...
          : undefined, // Changed from searchFields to search
        sort: { createdAt: "desc" },
      })
    );
  }, [
    ...
    if (newPage >= 1 && newPage <= pagination.totalPages) {
      dispatch(
        fetchTenants({
          ...
          search: searchInput
            ? { companyName: searchInput, subdomain: searchInput }
            : undefined, // Changed from searchFields to search// Changed from searchFields to search
          sort: { createdAt: "desc" },
        })
      );
    }
  ...
  const handleDeleteConfirm = async () => {
    if (categoryToDelete) {
      ...
      try {
        ...
        await dispatch(deleteTenant(categoryToDelete._id)).unwrap();

        setPopup({
          message: `Tenant "${categoryToDelete.name}" deleted successfully`,
          type: "success",
          isVisible: true,
        });
        ...
        console.log(`Tenant "${categoryToDelete.name}" deleted successfully`);
      } catch (error) {
  ...
  return (
    <div>
      <PageMeta
        title="Tenant List | TailAdmin"
        description="List of all tenants in TailAdmin"
      />
              ...
              {tenants.map((cat, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm capitalize">
                    {cat.subscriptionStatus !== "cancelled" ||
                    cat.subscriptionStatus !== "expired" ? (
                      <div className="px-2 py-1 bg-green-500/30 rounded-full flex justify-center">
                        {cat.subscriptionStatus}
                      </div>
                    ) : (
                      <div className="px-2 py-1 bg-red-500/30 rounded-full flex justify-center">
                        {cat.subscriptionStatus}
                      </div>
                    )}
                  </td>
